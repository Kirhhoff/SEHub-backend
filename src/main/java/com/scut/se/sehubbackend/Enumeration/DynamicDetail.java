...
/**
 * 用于动态权限的扩展<br/>
 * 在每个部门只有一种动态权限的时候，只需要{@link DepartmentEnum}即可区分不同的动态权限，而一旦有部门需要多种动态权限，就需要添加其他
 * 的区分信息，这也是此枚举的作用，部分描述可见{@link com.scut.se.sehubbackend.Security.Authorization.interfaces.AuthorityMapper#mapDynamic(DepartmentEnum, DynamicDetail)}.
