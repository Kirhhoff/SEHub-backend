...
public class HashAuthorityMapperTest {
    ...
    public void testMapPositionAndDepartment(){
        assertEquals(new SimpleGrantedAuthority("Minister_Research"),mapper.map(Position.Minister, DepartmentEnum.Research));//用部长做测试
        assertEquals(new SimpleGrantedAuthority("Staff_Research"),mapper.map(Position.Staff, DepartmentEnum.Research));//用部员做测试
    ...
    public void testMapDynamic(){
        assertEquals(new SimpleGrantedAuthority("Dynamic_Research"),mapper.mapDynamic(DepartmentEnum.Research,null));//动态部门权限
        assertEquals(null,mapper.mapDynamic(DepartmentEnum.Research, DynamicDetail.General));//试图生成不存在的权限时得到null
